...
ALTER TABLE content_calendars DROP CONSTRAINT IF EXISTS content_calendars_platform_check;
...
ALTER TABLE content_calendars DROP CONSTRAINT IF EXISTS content_calendars_status_check;
...
DROP POLICY IF EXISTS "analytics_reports_select_own" ON analytics_reports;
CREATE POLICY "analytics_reports_select_own" ON analytics_reports
  FOR SELECT USING (auth.uid() = client_id);
DROP POLICY IF EXISTS "analytics_reports_admin_all" ON analytics_reports;
CREATE POLICY "analytics_reports_admin_all" ON analytics_reports
  FOR ALL USING (public.is_admin());

DROP POLICY IF EXISTS "content_calendars_select_own" ON content_calendars;
CREATE POLICY "content_calendars_select_own" ON content_calendars
  FOR SELECT USING (auth.uid() = client_id);
DROP POLICY IF EXISTS "content_calendars_admin_all" ON content_calendars;
